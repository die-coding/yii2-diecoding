...
{
    const CACHE_TAG = 'die.rbac.route';
    ...
    public function getRoutePrefix()
    {
        if (!$this->_routePrefix) {
            $this->_routePrefix = Option::instance()->advanced ? self::PREFIX_ADVANCED : self::PREFIX_BASIC;
        }
    ...
    public function getRoutes()
    {
        ...
        $advanced = Option::instance()->advanced;
        if ($advanced) {
    ...
    public function getAppRoutes($module = null)
    {
        ...
        $cache = Option::instance()->cache;
        if ($cache === null || ($result = $cache->get($key)) === false) {
            $result = [];
            $this->getRouteRecursive($module, $result);
            if ($cache !== null) {
                $cache->set($key, $result, Option::instance()->cacheDuration, new TagDependency([
                    'tags' => self::CACHE_TAG,
                ]));
            }
        }
    ...
    public static function invalidate()
    {
        if (Option::cache() !== null) {
            TagDependency::invalidate(Option::cache(), self::CACHE_TAG);
        }
    }
    ...
    {
        if (Option::authManager()->getRule(RouteRule::RULE_NAME) === null) {
            Option::authManager()->add(new RouteRule());
        }
    }
}
