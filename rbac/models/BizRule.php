...
class BizRule extends \yii\base\Model
{
    ...
    public function classExists()
    {
        if (!class_exists($this->className)) {
            $message = Yii::t('diecoding', "Unknown class '{class}'", ['class' => $this->className]);
            $this->addError('className', $message);
            return;
        }
        if (!is_subclass_of($this->className, Rule::className())) {
            $message = Yii::t('diecoding', "'{class}' must extend from 'yii\rbac\Rule' or its child class", [
    ...
    public function attributeLabels()
    {
        return [
            ...
            'className' => Yii::t('diecoding', 'Class Name'),
        ];
    }
    ...
    {
        $item = Option::authManager()->getRule($id);
        if ($item !== null) {
