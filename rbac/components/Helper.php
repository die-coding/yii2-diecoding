...
class Helper
{
  ...
  protected static function getDefaultRoutes()
  {
    if (self::$_defaultRoutes === null) {
      ...
      if ($cache && ($routes = $cache->get($roles)) !== false) {
        ...
        if ($cache) {
          $cache->set($roles, self::$_defaultRoutes, Option::cacheDuration(), new TagDependency([
  ...
  public static function getRoutesByUser($userId)
  {
    if (!isset(self::$_userRoutes[$userId])) {
      ...
      if ($cache && ($routes = $cache->get([__METHOD__, $userId])) !== false) {
        ...
        self::$_userRoutes[$userId] = $routes;
        if ($cache) {
          $cache->set([__METHOD__, $userId], $routes, Option::cacheDuration(), new TagDependency([
            'tags' => Option::CACHE_TAG,
          ]));
        }
      }
  ...
  public static function checkRoute($route, $params = [], $user = null)
  {
    ...
    $r        = static::normalizeRoute($route, $instance->advanced);
    if ($instance->onlyRegisteredRoute && !isset(static::getRegisteredRoutes()[$r])) {
      return true;
    }
  ...
  protected static function normalizeRoute($route, $advanced = false)
  {
    ...
    } elseif (($uniqueId = Yii::$app->controller->module->getUniqueId()) !== '') {
      $normalized = '/' . $uniqueId . '/' . $route;
    } else {
      $normalized = '/' . $route;
    }
  ...
  public static function invalidate()
  {
    if (Option::cache() !== null) {
      TagDependency::invalidate(Option::cache(), Option::CACHE_TAG);
    }
  }
}
