...
class AuthItem extends Model
{
    ...
    {
        $name = $this->ruleName;
        if (!Option::authManager()->getRule($name)) {
            try {
                $rule = Yii::createObject($name);
                if ($rule instanceof \yii\rbac\Rule) {
                    $rule->name = $name;
                    Option::authManager()->add($rule);
                } else {
                    $this->addError('ruleName', Yii::t('diecoding', 'Invalid rule "{value}"', ['value' => $name]));
                }
            } catch (\Exception $exc) {
                $this->addError('ruleName', Yii::t('diecoding', 'Rule "{value}" does not exists', ['value' => $name]));
            }
        }
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'type' => Yii::t('diecoding', 'Type'),
            'description' => Yii::t('diecoding', 'Description'),
            'ruleName' => Yii::t('diecoding', 'Rule Name'),
    ...
    {
        $item = Option::authManager()->getRole($id);
        if ($item !== null) {
